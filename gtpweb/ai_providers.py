...
def build_model_options(openai_models: list[str], google_models: list[str]) -> tuple[ModelOption, ...]:
    options: list[ModelOption] = []
    for provider, model_names in (
        (PROVIDER_OPENAI, openai_models),
        (PROVIDER_GOOGLE, google_models),
    ):
        for model_name in model_names:
            options.append(build_model_option(provider, model_name))
    if not options:
